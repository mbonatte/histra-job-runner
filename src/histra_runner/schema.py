...
def _parse_outputs(data: Any, label: str) -> AnalysisOutputs:
    ...
    return AnalysisOutputs(
        ...
        modal_contributions=ModalOutputSpec(
            enabled=bool(modal_data.get("enabled", False)), top_n=top_n
        ),
    )
...
def job_spec_from_dict(data: Any) -> JobSpec:
    ...
    scoured = scour_data.get("scoured_foundation_interface_material", "Soil_removed")
    if not isinstance(scoured, str) or not scoured.strip():
        raise JobValidationError(
    ...
    minimum_bytes = validation_data.get("minimum_results_bytes", 1)
    ...
    return JobSpec(
        ...
        mesh=mesh,
        scour=ScourSpec(tuple(raw_materials), scoured),
        validation=ValidationSpec(
            ...
            minimum_results_bytes=minimum_bytes,
        ),
...
def load_job_spec(path: str | Path) -> JobSpec:
    return job_spec_from_dict(read_json(path))
