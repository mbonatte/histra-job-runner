    ...
    def execute_package(
        ...
        *,
        ...
        envelope = ResultEnvelope(
            ...
            attempt_id=package.manifest.attempt_id,
            job_sha256=package.manifest.job_sha256,
            hrx_sha256=package.manifest.hrx.sha256,
            results=execution.results,
            run=execution.run,
            logs=execution.logs,
        ).model_dump(mode="json")
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / "result-envelope.json").write_text(
            json.dumps(envelope, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        return RunOutcome(envelope=envelope, package=package, output_dir=output_dir)
